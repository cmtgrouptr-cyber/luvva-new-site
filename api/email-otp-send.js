...
function domainAllowed(email) {
  ...
  return configured.split(',').map(normalizeDomain).filter(Boolean).includes(domain);
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const configuredFromIsValid = validEmail(configuredMailbox) && configuredMailbox.endsWith('@luvva.tech');
    ...
    const from = (!configuredFromIsValid || configuredMailbox === email)
